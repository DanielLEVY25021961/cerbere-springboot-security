...
public abstract class AbstractGestionnaireRG implements IGestionnaireRG {
	...
	/**
	 * Map&lt;String, LigneRG&gt; contenant toutes les RG concernant l'OBJET METIER
	 ...
	 * [id;Actif;activité des contrôles sur l'attribut;activité de la RG ;RG
	 * implémentée;clé du type de contrôle;type de contrôle ;Message d'erreur;Objet
	 * Métier concerné;Attribut concerné ;Classe implémentant la RG;Méthode
	 * implémentant la RG; properties;clé;].<br/>
	 */
	protected final transient Map<String, LigneRG> mapRG = new LinkedHashMap<String, LigneRG>();

	/**
	 * bundleExterneRG : ResourceBundle :<br/>
	 * ResourceBundle encapsulant objet_RG.properties.<br/>
	 * objet_RG.properties est un fichier EXTERNE (hors classpath) qui doit être
	 * accessible à la Maîtrise d'Ouvrage (MOA).<br/>
	 */
	 ...
	 */
	private static final Log LOG = LogFactory.getLog(AbstractGestionnaireRG.class);
	...
	/**
	 * <ul>
	 * <li>Fournit le ResourceBundle associé au fichier <i>externe</i> (hors
	 * classpath) <b>objet_RG.properties</b> avec la Locale pLocale.</li>
	 * </ul>
	 * <br/>
	 *
	 ...
	 * @throws Exception
	 */
	private ResourceBundle getBundleExterneRG(final Locale pLocale) throws Exception {

		ResourceBundle resourceBundle = null;

		/*
		 * Accède au répertoire externe contenant objet_RG.properties.
		 */
		final File repertoireRessourcesParametrables = new File(fournirCheminRessourceExterneRG());
		...
		resourceBundle = ResourceBundle.getBundle(this.fournirNomBasePropertiesRG(), pLocale,
	...
	/**
	 * <ul>
	 * remplit et retourne la Map&lt;String, LigneRG&gt; mapRG contenant toutes les
	 * Règles de Gestion (RG) de l'UTILISATEUR implémentées dans les services de
	 * l'application avec :
	 * <li>String : le nom de la RG.</li>
	 * <li>LigneRG : pure fabrication encapsulant tous les éléments relatifs à la
	 * RG.</li>
	 * </ul>
	...
	/**
	 * <ul>
	 * <li>fournit le chemin <b>externe</b> (hors classpath) du <b>répertoire</b>
	 * contenant le fichier <b>objet_RG.properties</b>.</li>
	 * <li>Ce chemin doit être écrit <b>EN ABSOLU</b> (surtout pas relatif au projet
	 * Eclipse).</li>
	 * <li>Par exemple
	 * <code><b>H:.../ressources_externes/rg/metier/usersimple</b></code> pour le
	 * fichier "usersimple_RG_fr_FR.properties".</li>
	 * </ul>
	 *
	 * @return : String : chemin absolu vers le File objet_RG.properties.<br/>
	 * 
	 * @throws Exception
	 */
	protected abstract String fournirCheminRessourceExterneRG() throws Exception;

	/**
	 * <ul>
	 * <li>fournit le <b>nom de base</b> du objet_RG.properties en fonction de
	 * l'objet traité par le <b>GestionnaireRGObjet</b>.</li>
	...
	public final String getEnTeteCsv() {
		...
		return "id;Actif;activité des contrôles sur l'attribut;"
				+ "activité de la RG;RG implémentée;clé du type de contrôle;" + "type de contrôle;"
				+ "Message d'erreur;Objet Métier concerné;" + "Attribut concerné;Classe implémentant la RG;"
...
//		final Set<Entry<String, LigneRG>> entrySet = mapTriee.entrySet();
	 ...
	 */
	@Override
	public final ResourceBundle getBundleExterneRG() throws Exception {

		if (this.bundleExterneRG == null) {
			this.bundleExterneRG = this.getBundleExterneRG(LocaleManager.getLocaleApplication());
		}
	 ...
	 */
	@Override
	public final Path getPathAbsoluPropertiesRG() throws Exception {

		Path resultat = null;

		final Path pathAbsoluRepProperties = Paths.get(this.fournirCheminRessourceExterneRG());
		...
		final String localeAppliquee = this.getExtensionLangageCountryLocale(localeBundle);

		String nomProperties = null;

		if (StringUtils.isBlank(localeAppliquee)) {
			nomProperties = nomBaseProperties + localeAppliquee + ".properties";
		} else {
			nomProperties = nomBaseProperties + "_" + localeAppliquee + ".properties";
		}

		final Path pathRelatifFichier = Paths.get(nomProperties);

		resultat = pathAbsoluRepProperties.resolve(pathRelatifFichier).toAbsolutePath().normalize();
	 ...
	 * - retourne une chaîne vide si pLocale.getDisplayName() est blank (cas ou le
	 * properties n'a pas d'extension de langue-pays comme dans
	 * 'application.properties').<br/>
	 * <br/>
	 *
	 * @param pLocale : Locale.
	 * 
	 * @return : String : extension comme "fr_FR" dans
	 *         <code><b>application_fr_FR.properties</b></code>.<br/>
	 */
	private String getExtensionLangageCountryLocale(final Locale pLocale) {
		...
		if (StringUtils.isBlank(pLocale.getDisplayName(LocaleManager.getLocaleApplication()))) {
			return "";
		}

		final String resultat = this.getLangageLocale(pLocale) + UNDERSCORE_STRING + this.getCountryLocale(pLocale);
	 ...
	 * Par exemple : "fr" dans <code><b>fr_FR</b></code> ou "en" dans
	 * <code><b>en_US</b></code>.<br/>
	 * <br/>
	 * - retourne null si pLocale == null.<br/>
	 * <br/>
	 *
	 * @param pLocale : Locale.<br/>
	 * 
	 * @return : String : Le langage (fr, en, ...) de pLocale.<br/>
	 */
	private String getLangageLocale(final Locale pLocale) {
	 ...
	 * Par exemple : "FR" dans <code><b>fr_FR</b></code> ou "US" dans
	 * <code><b>en_US</b>/code>.<br/>
	 * <br/>
	 * - retourne null si pLocale == null.<br/>
	 * <br/>
	 ...
	 * @param pLocale : Locale.<br/>
	 * 
	 * @return : String : le country (FR, US, ...) de pLocale.<br/>
	 */
	private String getCountryLocale(final Locale pLocale) {
