...
//		expliciterURL(url1);
	...
	/**
	 * retourne l'<b>adresse IP privée externe LOCALE de la présente machine au sein
	 * du réseau LOCAL</b>.
	 * <ul>
	 * <li>Par exemple 10.77.222.142 sur le réseau local wifidtecitm pour la
	 * présente machine localhost.</li>
	 * <li>adresse <b>interne</b> au réseau local.<br/>
	 * Cette adresse IP n'est <b>pas visible depuis l'Internet</b>.<br/>
	 * Elle sert à la conversation entre le routeur local et la présente
	 * machine.</li>
	 ...
	 */
	public static InetAddress connaitreAdressePriveeLocale() throws UnknownHostException {
	 ...
	 * @throws UnknownHostException
	 */
	public static InetAddress connaitreServeurParIp(final String pAdresseIp) throws UnknownHostException {
	...
	/**
	 * retourne l'adresse IP et les informations du serveur nommé pHostName.<br/>
	 * <br/>
	 ...
	 * @throws UnknownHostException
	 */
	public static InetAddress connaitreAdresseIPparNomServeur(final String pHostName) throws UnknownHostException {
	...
	/**
	 * Ouvre un Scanner (Console) permettant de tester des adresses IP ou des noms
	 * de domaine.<br/>
	 * <ul>
	 ...
	 * <li>retourne parfois l'adresse IP à la place du nom de domaine si celui-ci
	 * n'est pas renseigné sur le serveur pHost.</li>
	 * </ul>
	 * 
	 * @throws Exception
	 */
		...
		try {
			...
			while (true) {
				...
				System.out.println(
						"-----------------------------------------------------------------------------------------------------------------------------");
				System.out.println(
						"Saisissez dans la console une adresse IPV4 ou un nom de domaine (ou tapez 'fin' dans la console pour arrêter le programme) : ");
				System.out.println(
						"-----------------------------------------------------------------------------------------------------------------------------");
	 ...
	 * </ul>
	 *
	 * @param pHost : String : nom de l'hôte (serveur) sous forme d'adresse IP ou de
	 *              nom de domaine
	 * 
	 * @return : String : adresse IP ou nom de domaine.<br/>
	 */
	public static String lookup(final String pHost) {
	 ...
	 * @throws UnknownHostException
	 */
	public static InetAddress[] connaitreToutesAdressesIPparNomServeur(final String pHostName)
			throws UnknownHostException {
	 ...
	 *
	 * @param pUrl : java.net.URL : url d'un site comme par exemple
	 *             "http://www.google.fr:80".<br/>
	 */
	public static void expliciterURL(final URL pUrl) {
	...
	/**
	 * affiche les informations sur toutes les cartes réseau de la présente
	 * machine.<br/>
	 * 
	 ...
	 */
	public static void calculerPrefixeAdresseMasque(final String pString) {
		...
		byte[] bytesMasque = new byte[] { (byte) (valeur >>> 24), (byte) (valeur >> 16 & 0xff),
				(byte) (valeur >> 8 & 0xff), (byte) (valeur & 0xff) };
		...
		try {
			...
			final byte[] bytesIp = new byte[] { (byte) ((ipl >> 24) & 0xFF), (byte) ((ipl >> 16) & 0xFF),
					(byte) ((ipl >> 8) & 0xFF), (byte) (ipl & 0xFF) };

			// Le ET logique entre l'pString IP et le masque
			final byte[] bytesReseau = new byte[] { (byte) (bytesIp[0] & bytesMasque[0]),
					(byte) (bytesIp[1] & bytesMasque[1]), (byte) (bytesIp[2] & bytesMasque[2]),
			...
			// inverser le masque
			bytesMasque = new byte[] { (byte) (~bytesMasque[0] & 0xff), (byte) (~bytesMasque[1] & 0xff),
					(byte) (~bytesMasque[2] & 0xff), (byte) (~bytesMasque[3] & 0xff), };

			System.out
					.println("Masque inverse (Wildcard) =\t" + InetAddress.getByAddress(bytesMasque).getHostAddress());

			final byte[] bytesBroadcast = new byte[] { (byte) (bytesReseau[0] | bytesMasque[0]),
					(byte) (bytesReseau[1] | bytesMasque[1]), (byte) (bytesReseau[2] | bytesMasque[2]),
	 ...
	 * @param pName    : String
	 */
	public static void showInformations(final InetAddress pAddress, final String pName) {
	 ...
	 * @param pName      : String.
	 */
	public static void showInformations(final InetAddress[] pAddresses, final String pName) {
